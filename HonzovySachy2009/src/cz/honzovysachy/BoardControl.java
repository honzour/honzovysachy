...
public class BoardControl extends View {
	...
	protected boolean hrajeClovek() {
		return !isPremyslim()
				&& (mSavedTaskAndroid.mWhitePerson
						&& mTask.mBoardComputing.bily || mSavedTaskAndroid.mBlackPerson
						&& !mTask.mBoardComputing.bily);
	}
	...
	public boolean trySave() {
		try {
			...
			o.writeObject(new SavedTaskChessAndroid(mSavedTaskAndroid, mTask
					.getSavedTask()));
			o.close();
		} catch (Exception e) {
			return false;
	...
	public void tahni(int tah) {
		...
		if (mTask.mEnd != 0)
			dlg(mTask.getEndOfGameString(mTask.mEnd));
		invalidate();
		pripravTah();
	...
	public void replayPromotion(int type) {
		...
		int tah = mTask.makeMove(t, mFieldFrom, mFieldTo,
				new PawnPromotionGUIQueen(type + 2));
		tahni(tah);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getAction() != MotionEvent.ACTION_DOWN)
			return false;
		if (mSavedTaskAndroid.mPole <= 0 || !hrajeClovek())
			return false;
		...
		int pole1 = Pozice.a1 + mSavedTaskAndroid.mox + 10
				* mSavedTaskAndroid.moy;
		if (mTask.JeTam2(t, pole1, pole)) {
			if (Math.abs(mTask.mBoardComputing.sch[pole1]) == 1
					&& (y == 7 || y == 0)) {
				mFieldFrom = pole1;
				mFieldTo = pole;
			...
			int tah = mTask.makeMove(t, pole1, pole,
					new PawnPromotionGUIQueen());
			tahni(tah);
			return true;
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < 8; i++)
			for (int j = 0; j < 8; j++) {
				Paint p;
				if (clovek && mSavedTaskAndroid.mox == i
						&& mSavedTaskAndroid.moy == j) {
					p = zelena;
				} else {
					p = ((((i + j) & 1) == 1) ? bila : cerna);
				}
				int sx = (mSavedTaskAndroid.mFlipped ? 7 - i : i)
						* mSavedTaskAndroid.mPole;
				int sy = (mSavedTaskAndroid.mFlipped ? j : 7 - j)
						* mSavedTaskAndroid.mPole;
				canvas.drawRect(new Rect(sx, sy, sx + mSavedTaskAndroid.mPole,
						sy + mSavedTaskAndroid.mPole), p);
				byte co = sch[Pozice.a1 + i + 10 * j];

				if (co != 0 && co > -7 && co < 7) {
					Drawable dr = mFigury[co > 0 ? 1 : 0][co > 0 ? co - 1
							: -co - 1];
					dr.setBounds(sx, sy, sx + mSavedTaskAndroid.mPole, sy
							+ mSavedTaskAndroid.mPole);
					dr.draw(canvas);
				}
			}
		...
		Thread t = new Thread() {
			public void run() {
				...
				SharedPreferences pref = mActivity.getSharedPreferences(
						AktivitaSachovnice.SETTINGS, 0);
				int time = 5000;
				tah = Minimax.minimax(
						mTask,
						pref == null ? time : pref.getInt(
								AktivitaSachovnice.TIME_PER_MOVE, time),
						new ThinkingOutput() {
							int mDepth = 0;
							String mMove = "";
							int mValue = 0;
							...
							private void draw() {
								mHandler.post(new Runnable() {
									public void run() {
										TextView v = (TextView) mActivity
												.findViewById(R.id.move);
										v.setText(mMove + " (" + mDepth + ") "
												+ mValue);
									}
								});
							}
	...
	protected void setPersonsAfterUndoRedo() {
		boolean hh = mSavedTaskAndroid.mWhitePerson
				&& mSavedTaskAndroid.mBlackPerson;
		if (!hh) {
	...
	public void save(Intent data) {
		try {
			...
			try {
				pgnData = (PGNHeaderData) data
						.getSerializableExtra("PGNHeader");
			} catch (Exception e) {
				return;
			}
			if (pgnData == null)
				return;
			mTask.savePNG(new DiacriticsFreePrintWriter(pgnData.mFileName),
					true, pgnData);
	...
	public void setupBoard(boolean useBoardComputing) {
		...
		CheckBox bWhite = (CheckBox) mActivity
				.findViewById(R.id.board_setting_white);
	...
	public void setupBoardOK(boolean cancel) {
		...
		if (cancel) {
			...
			if (mTask.mBoard.sch[Pozice.e1] == 6) {
				if (mTask.mBoard.sch[Pozice.h1] == 4)
					mTask.mBoard.roch |= 1;
				if (mTask.mBoard.sch[Pozice.a1] == 4)
					mTask.mBoard.roch |= 2;
			}
			if (mTask.mBoard.sch[Pozice.e8] == -6) {
				if (mTask.mBoard.sch[Pozice.h8] == -4)
					mTask.mBoard.roch |= 4;
				if (mTask.mBoard.sch[Pozice.a8] == -4)
					mTask.mBoard.roch |= 8;
			}
